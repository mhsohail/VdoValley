    ...
    public class VideosController : Controller
    {
        ...
        public ActionResult Details(int? id)
        {
            ...
            if (video.Url == null)
            {
                ...
                {
                    ViewBag.OgThumbnail = VdoValley.Web.Helpers.VideoHelper.getFacebookVideoThumbnail(video, VdoValley.Web.Helpers.VideoHelper.getFacebookVideoCode(video, video.EmbedCode), VdoValley.Core.Models.FacebookThumbnailSize.MEDIUM).Replace("amp;", string.Empty);
            ...
            else
            {
                if (video.ThumbnailURL == null)
                {
                    ViewBag.OgThumbnail = VideoHelper.getDailyMotionThumb(video, VideoHelper.getDailyMotionVideoCode(video, video.Url), VdoValley.Core.Models.DailymotionThumbnailSize.thumbnail_large_url);
            ...
            VdoValley.Core.Models.Rating VideoScorer = db.Ratings.FirstOrDefault(r => r.ApplicationUserId.Equals(LoggedInUserId) && r.VideoId.Equals(video.VideoId));
            if (VideoScorer != null)
            {
                ...
                using (DbContextTransaction transaction = db.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        */
                        ...
                        /*
                        List<Tagg> tagsToAdd = new List<Tagg>();
                        foreach (var tag in tags)
                        {
                            Tagg newTag = new Tagg();
                            newTag.Name = tag;
                            tagsToAdd.Add(newTag);
                        }
                        */
